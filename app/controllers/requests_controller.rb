class RequestsController < ApplicationController
	...
	def index
		...
		if params[:search]
			@requests = Request.search(params[:search], params[:category], params[:city]).order("created_at DESC").paginate(:page => params[:page], :per_page => 5)
		elsif params[:city]
			@requests = Request.all.where(params[:city]).paginate(:page => params[:page], :per_page => 5)
		elsif params[:category]
			@requests = Request.all.where(params[:category]).where(params[:city]).paginate(:page => params[:page], :per_page => 5)
		else
